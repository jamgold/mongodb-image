...
(function($) {
    ...
    $.widget('ui.tagit', {
        options: {
            ...
            defaultClasses: 'tagit-choice ui-widget-content ui-state-default ui-corner-all',
            existingEffect: 'highlight',
            existingReset: true,
                ...
                .click(function(e) {
                    ...
                    if (target.hasClass('tagit-label')) {
                        ...
                        if (!tag.hasClass('removed')) {
                            ...
                            that._trigger('onTagClicked', e, {tag: tag, tagValue: value, tagLabel: that.tagLabel(tag)});
                        }
                    } else {
        ...
        createTag: function(data, additionalClass, duringInitialization) {
            ...
            if(that.options.debug) console.debug(`createTag label=${tagLabelValue} value=${value}`);
            ...
            if (!this.options.allowDuplicates && !this._isNew(tagLabelValue)) {
                ...
                if (this._trigger('onTagExists', null, {
                    ...
                    // console.log(tagLabelValue, existingTag, this.options);

                    if (this._effectExists(this.options.existingEffect)) {
                        existingTag.effect(this.options.existingEffect);
                        if (this.options.existingReset) this.tagInput.val('');
                    }else{
                        console.log(`${this.options.existingEffect} effect does not exist`);
                    }
                }
                return false;
            }
            ...
            // Create tag.
            var tag = $('<li></li>')
                .addClass(this.options.defaultClasses)
        ...
        removeTag: function(tag, animate) {
            ...
            if (animate) {
                ...
                hide_args.push(function() {
                    tag.remove();
                    thisTag._trigger('afterTagRemoved', null, {tag: tag, tagValue: value, tagLabel: thisTag.tagLabel(tag)});
                });

                tag.fadeOut('fast').hide.apply(tag, hide_args).dequeue();
            } else {
                tag.remove();
                this._trigger('afterTagRemoved', null, {tag: tag, tagValue: value, tagLabel: this.tagLabel(tag)});
            }
